...
- MonthFormat: MonthFormatPreset
  $name: Month Format Preset
  $options:
  - Amphoreus: Use Amphoreus Names
  - custom: Custom, specified below
- MonthFormatCustom: >-
    Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Feb, Oct, Nov, Dec
  $name: Custom month format
  $description: >-
    Used with the %month_custom% pattern.
- WordsFormat: Amphoreus
  $name: Month Words Preset
  $options:
  - Amphoreus: Use Amphoreus Settings
  - custom: Custom, specified below
- WordsForMonths: Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Feb, Oct, Nov, Dec, NewYear
  $name: Custom words per month
  $description: >-
    Used with the %words% pattern.
*/
// ==/WindhawkModSettings==
...
struct {
    ...
    // Customed Month
    StringSetting monthFormat;
    std::vector<std::wstring> monthFormatCustom;
    std::vector<std::wstring> wordsForMonths;
} g_settings;
...
size_t ResolveFormatToken(
    ...
    // Add inside ResolveFormatToken lambda
    if (auto token = L"%month_custom%"sv; format.starts_with(token)) {
        SYSTEMTIME time = g_formatTime;
        if (g_settings.monthFormatCustom.size() > time.wMonth - 1 && 
            !g_settings.monthFormatCustom[time.wMonth - 1].empty()) {
            resolvedCallback(g_settings.monthFormatCustom[time.wMonth - 1].c_str());
        } else {
            resolvedCallback(L"-");
        }
        return token.size();
    ...
    if (auto token = L"%day%"sv; format.starts_with(token)) {
        WCHAR buffer[4];
        swprintf_s(buffer, L"%02d", g_formatTime.wDay);
        resolvedCallback(buffer);
        return token.size();
    }

    // Full year (e.g., 2025)
    if (auto token = L"%year%"sv; format.starts_with(token)) {
        WCHAR buffer[8];
        swprintf_s(buffer, L"%d", g_formatTime.wYear);
        resolvedCallback(buffer);
        return token.size();
    }

    // Short year (e.g., 25)
    if (auto token = L"%year_short%"sv; format.starts_with(token)) {
        WCHAR buffer[4];
        swprintf_s(buffer, L"%02d", g_formatTime.wYear % 100);
        resolvedCallback(buffer);
        return token.size();
    }

    if (auto token = L"%words%"sv; format.starts_with(token)) {
        SYSTEMTIME time = g_formatTime;
        int idx;

        // 特殊逻辑：12月31日 或 1月1日 → 使用第12个字段（索引11）
        if ((time.wMonth == 12 && time.wDay == 31) ||
            (time.wMonth == 1 && time.wDay == 1)) {
            idx = 12; // 第13个字段
        } else {
            idx = time.wMonth - 1; // 正常按月份：1月→0, ..., 12月→11
        }

        if (idx >= 0 && idx < 12 && !g_settings.wordsForMonths[idx].empty()) {
            resolvedCallback(g_settings.wordsForMonths[idx].c_str());
        } else {
            resolvedCallback(L"-");
        }
        return token.size();
...
g_settings.monthFormat = StringSetting::make(L"MonthFormat");
g_settings.monthFormatCustom.clear();

if (wcscmp(g_settings.monthFormat, L"Amphoreus") == 0) {
    // Amphoreus 预设
    static const std::vector<std::wstring_view> kMythicMonths = {
        L"门关月", L"平衡月", L"长夜月", L"耕耘月", L"欢喜月", L"长昼月",
        L"自由月", L"收获月", L"拾线月", L"纷争月", L"哀悼月", L"机缘月"
    };
    g_settings.monthFormatCustom.assign(kMythicMonths.begin(), kMythicMonths.end());
} else {
    // custom mode
    StringSetting monthFormatCustom = StringSetting::make(L"MonthFormatCustom");
    if (*monthFormatCustom) {
        for (const auto monthName : SplitStringView(monthFormatCustom.get(), L",")) {
            g_settings.monthFormatCustom.emplace_back(TrimStringView(monthName));
        }
    }
}
g_settings.monthFormatCustom.resize(12, L"");

// Load WordsForMonths based on WordsFormat
g_settings.wordsForMonths.clear();

StringSetting wordsFormat = StringSetting::make(L"WordsFormat");
bool useAmphoreus = (wcscmp(wordsFormat, L"Amphoreus") == 0);

if (useAmphoreus) {
    // 🌟 Amphoreus preset
    static const std::vector<std::wstring_view> kAmphoreusWords = {
        L"提里西庇俄斯 Tribios ：⌈不论晴天还是雨天，进入梦乡以前，记得和自己说一声：『明天见』！⌋",
        L"刻律德菈  Cerydra ：⌈无我，即无世界。去思，去见，去征服。⌋",
        L"长夜月  Evernight ：⌈如果岁月是个无穷的轮回——那就忘掉不断重复的苦涩，单独记住每一个快乐的瞬间吧！⌋",
        L"丹恒·腾荒  Dan Heng · Permansor Terrae ：⌈笔终会折断，墨终会耗尽，生命终会逝去，单思想将在史诗中永垂不朽。⌋",
        L"海瑟音  Hysilens ：⌈世事如沧海，潮起潮落，一切终会在浪中消逝。所以，请在生命的每一刻都纵情高歌吧。⌋",
        L"雅辛忒丝 Hyacinthia ：⌈多望望天，多笑一笑；好多缠人的病痛，最怕乐观这剂良药！⌋",
        L"白厄 Phainon ：⌈每个人的心中都住在一个英雄。拥抱它，然后去追逐太阳吧。⌋",
        L"阿那克萨戈拉斯 Anaxagoras ：⌈『真理是溶解世界万物的溶剂，因而绝无法在客观上存在』——对此，本人心中已有绝妙的证明。⌋",
        L"阿格莱雅 Aglaea ：⌈信任与无私，是世上最美丽的两件华服：一键赠予他人，一件装扮自己。⌋",
        L"迈德漠斯 Mydeimos ：⌈去杀死晚刷，游戏便会诞生。去杀死纷争，荣光必然降临。⌋",
        L"遐蝶 Castorice ：⌈若心灵仍有余裕，请稍许分出些温柔，轻抚身边美好的一切吧。⌋",
        L"赛法利娅 Cifera ：⌈别怕摔，跑起来！命运就是只迟缓的若虫，它压根抓不住你！⌋"
    };
    g_settings.wordsForMonths.assign(kAmphoreusWords.begin(), kAmphoreusWords.end());
} else {
    // custom mode
    StringSetting wordsForMonths = StringSetting::make(L"WordsForMonths");
    if (*wordsForMonths) {
        for (const auto word : SplitStringView(wordsForMonths.get(), L",")) {
            g_settings.wordsForMonths.emplace_back(TrimStringView(word));
        }
    }
}
g_settings.wordsForMonths.resize(13, L"");
